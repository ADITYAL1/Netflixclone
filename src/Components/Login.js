  ...
  const navigate=useNavigate();
   ...
   return (
     <div>
        ...
        <form onSubmit={(e)=>e.preventDefault()} className=' w-3/12 absolute p-12 bg-black normal-case   my-32 mx-auto right-0 left-0 text-white rounded-lg bg-opacity-80 font-semibold'> 
        ...
        <p className='py-2 ml-2 text-center text-sm font-semibold cursor-pointer hover:underline' onClick={isSignIn}> {SignIn ?'New to Netflix? Sign Up Now':'Already Registered? Sign In Now' }</p>
       </form>
     </div>
   )
 }
